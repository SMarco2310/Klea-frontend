...
<template>
  <div>
    <AppHeader :title="$t('nav.webhooks')" :subtitle="$t('webhooks.subtitle')" />
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
      <div class="p-5 rounded-xl bg-[var(--color-surface)] border border-[var(--color-border-dark)]">
        <div class="flex items-center gap-3 mb-3">
          ...
          <div class="min-w-0">
            <div class="font-medium text-sm">{{ $t('webhooks.endpointCard.title') }}</div>
            <div class="text-xs text-slate-500">{{ $t('webhooks.endpointCard.description') }}</div>
          </div>
        </div>
        <div
          v-if="endpointUrl"
        ...
        >
          ...
          <button class="text-slate-400 hover:text-white cursor-pointer shrink-0" :aria-label="$t('webhooks.endpointCard.copyAria')" @click="copyUrl">
            <component :is="copiedUrl ? CheckIcon : CopyIcon" class="w-3.5 h-3.5" />
          </button>
        </div>
        <p v-else class="text-xs text-slate-500">
          <i18n-t keypath="webhooks.endpointCard.notConfigured" tag="span">
            <template #link>
              <NuxtLink :to="`/${$route.params.workspaceSlug}/apps/${currentApp?.slug}/settings`" class="text-[var(--color-accent)] hover:underline">{{ $t('webhooks.endpointCard.setInSettingsLink') }}</NuxtLink>
            </template>
          </i18n-t>
        </p>
      </div>
      ...
      <div class="p-5 rounded-xl bg-[var(--color-surface)] border border-[var(--color-border-dark)]">
        <div class="flex items-center gap-3 mb-3">
          ...
          <div class="min-w-0">
            <div class="font-medium text-sm">{{ $t('webhooks.secretCard.title') }}</div>
            <i18n-t keypath="webhooks.secretCard.description" tag="div" class="text-xs text-slate-500">
        ...
        >
          ...
          <button
            ...
            :aria-label="secretVisible ? $t('webhooks.secretCard.hideAria') : $t('webhooks.secretCard.showAria')"
            @click="secretVisible = !secretVisible"
          >
            <component :is="secretVisible ? EyeOffIcon : EyeIcon" class="w-3.5 h-3.5" />
          </button>
          <button class="text-slate-400 hover:text-white cursor-pointer shrink-0" :aria-label="$t('webhooks.secretCard.copyAria')" @click="copySecret">
            <component :is="copiedSecret ? CheckIcon : CopyIcon" class="w-3.5 h-3.5" />
          </button>
        </div>
        <p v-else class="text-xs text-slate-500">{{ $t('webhooks.secretCard.autoGenerated') }}</p>
      </div>
    </div>

    <div class="rounded-xl bg-[var(--color-surface)] border border-[var(--color-border-dark)] p-0">
      <h3 class="text-sm font-medium text-slate-300 mb-4 p-5 pb-0">{{ $t('webhooks.deliveries.title') }}</h3>
      <div v-if="deliveries.length === 0" class="p-5">
        <EmptyState
          :icon="WebhookIcon"
          :title="$t('webhooks.deliveries.emptyTitle')"
      ...
      <table v-else class="w-full text-left text-sm whitespace-nowrap mt-4">
        <thead class="border-y border-[var(--color-border-dark)] bg-[var(--color-surface-muted)]">
          <tr>
            <th class="px-5 py-3 font-medium text-slate-300">{{ $t('webhooks.deliveries.columns.date') }}</th>
            <th class="px-5 py-3 font-medium text-slate-300">{{ $t('webhooks.deliveries.columns.status') }}</th>
            <th class="px-5 py-3 font-medium text-slate-300">{{ $t('webhooks.deliveries.columns.event') }}</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-[var(--color-border-dark)]">
          <tr v-for="delivery in deliveries" :key="delivery.id" class="hover:bg-white/5 transition-colors">
            ...
            <td class="px-5 py-3">
              ...
              >
                {{ delivery.status_code || $t('webhooks.deliveries.failedStatus') }}
              </span>
            </td>
            <td class="px-5 py-3 font-mono text-xs text-slate-300">
              {{ delivery.payload ? 'subscription.updated' : $t('webhooks.deliveries.unknownEvent') }} <!-- We could parse JSON here if needed -->
            </td>
          </tr>
        </tbody>
      </table>
