    ...
    <div class="grid grid-cols-1 lg:grid-cols-[1.1fr_1fr] gap-16 items-center">
      <ScrollReveal direction="up" :delay="100" :duration="800">
        <div>
          ...
          <h1 class="font-heading text-5xl md:text-6xl font-bold leading-[1.05] tracking-tight">
            ...
            <span class="text-[var(--color-accent)]">{{ $t('landing.hero.titleAccent') }}</span>
          </h1>
          ...
          <div class="flex flex-wrap items-center gap-4 mt-10">
            ...
            >
              <NuxtLink to="/signup" class="flex items-center gap-2">
                {{ $t('landing.hero.ctaPrimary') }} <ArrowRightIcon class="w-4 h-4" />
              </NuxtLink>
            </Button>
            <Button size="lg" variant="secondary" as-child class="cursor-pointer h-12 px-7 text-base">
              <NuxtLink to="/docs">{{ $t('landing.hero.ctaSecondary') }}</NuxtLink>
            </Button>
          </div>
          <p class="text-xs text-muted-foreground/80 mt-4 font-mono">{{ $t('landing.hero.noCreditCard') }}</p>
        </div>
      </ScrollReveal>
      ...
      <ScrollReveal direction="left" :delay="300" :duration="900">
        <div class="relative">
          ...
          <TerminalWindow :title="$t('landing.hero.terminalTitle')" class="relative shadow-2xl">
            ...
            <div class="mt-4 flex items-center gap-2 h-5">
              <template v-if="!validated">
                ...
                <span class="text-muted-foreground/80">{{ $t('landing.hero.terminalChecking') }}</span>
              </template>
              <template v-else>
                <CheckIcon class="w-4 h-4 text-[var(--color-accent)]" />
                <span class="text-[var(--color-accent)]">{{ $t('landing.hero.terminalValid') }}</span>
