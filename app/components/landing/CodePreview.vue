...
<template>
  <section class="max-w-3xl mx-auto px-6 py-24">
    <div class="text-center mb-10">
      <span class="text-xs font-mono text-[var(--color-accent)] uppercase tracking-widest">{{ $t('landing.codePreview.eyebrow') }}</span>
      <h2 class="font-heading text-2xl md:text-3xl font-semibold mt-3 tracking-tight">{{ $t('landing.codePreview.title') }}</h2>
    </div>
    <div class="relative">
      <div class="absolute -inset-8 bg-[var(--color-accent)]/[0.06] blur-3xl rounded-full" aria-hidden="true" />
      <TerminalWindow :title="$t('landing.codePreview.terminalTitle')" class="relative shadow-2xl">
